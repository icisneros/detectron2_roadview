...
def parse_args():
    ...
    parser.add_argument("--get_roadside", action="store_false")
...
def main(args):
    ...
    if args.get_incabin: videoWriter1 = cv2.VideoWriter(args.output_path+'incabin_{}.mp4'.format(ind), cv2.VideoWriter_fourcc(*'mp4v'), output_fps, (1280,720))
    if args.get_roadside: videoWriter2 = cv2.VideoWriter(args.output_path+'road_{}.mp4'.format(ind), cv2.VideoWriter_fourcc(*'mp4v'), output_fps, (1280,720))
    if args.get_speed: videoWriter3 = cv2.VideoWriter(args.output_path+'speed_{}.mp4'.format(ind), cv2.VideoWriter_fourcc(*'mp4v'), output_fps, (332,720))
